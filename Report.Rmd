...
```{r pairs_plot2, echo=FALSE, eval = FALSE, include=TRUE, warning=FALSE, message=FALSE, fig.width=16, fig.height=16, dpi=150, out.width='100%'}
...
ggplot(box_long, aes(x = variable, y = value, fill = variable)) +
  ...
  labs(x = NULL, y = "Scaled value",
       title = "Serbian Spa Waters: Boxplots of All Variables") +
...
par(family = "serif")
X <- X[, !names(X) == ".row"] # remove '.row' column
...
```{r echo=FALSE, include=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
...
ggplot(pca_plot_data, aes(x = PC1, y = PC2, color = Geological_Group)) +
  ...
  stat_ellipse(level = 0.95, alpha = 1, linewidth = 1) +  
  labs(
...
# Confusion matrix
cm <- table(True = y, Predicted = pred)
...
for (i in seq_along(classes)) {
  ...
  metrics[i, 2:5] <- round(c(precision, recall, f1, support), 3)
}
