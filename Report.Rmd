...
# Plot PC1 vs PC2, coloured by geological structure
ggplot(scores, aes(x = PC1, y = PC2, colour = geoStruct, label = waterSource)) +
  geom_point(size = 3) +
  geom_text(aes(label = ifelse(abs(PC1) > 3 | abs(PC2) > 3, waterSource, "")),
            vjust = -0.5, size = 3) +
  labs(title = "PCA score plot: PC1 vs PC2",
       x = "PC1 (overall mineralisation)",
       y = "PC2 (hardness vs temp/pH)")
```
...
# Factor Analysis - do in W11

# geoStruct groups - answer
