...
1. The classes are linearly separable 2. The covariance matrices are not too different.

In other words, within each geoStruc group the predictors should be approximately normally distributed. The covariance matrices must be the same for all geoStruc groups. Looking at the plot, the four groups form recognisable clusters: Group 1 (hydrological basins) is more spread out, Group 2 (karstic terrains) is very compact, Group 3 (volcanogen massifs) overlaps a little with other groups, and Group 4 (metamorphic regions) is quite separate on LD2. The ellipses are *fairly* similar in shape and orientation, which supports the equal covariance assumption, and the clear separation of some groups suggests LDA should work reasonably well here.
