...
```{r PCA-loadings-table, fig.cap="Serbian Spa Variable Influence on Principal Components", echo=FALSE, message=FALSE, warning=FALSE}
# Prepare loadings
...
kable(loadings_df, format = "latex", digits = 3, align = "r",
      caption = "Serbian Spa Variable Influence on Principal Components") %>%
  kable_styling(latex_options = c("HOLD_position"), full_width = FALSE) %>%
...
```{r pcaWithOutliers, fig.cap="PCA with Outliers Highlighted", echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r  groupVarCheck, fig.cap="Fig. 6: Visual Check of Group Covariance", echo=FALSE}
...
```{r fig.cap="Confusion Matrix for Naive Bayes Classification", echo=FALSE}
# pred needs matching levels
pred <- factor(pred, levels = levels(y))

# confusion table: rows = Predicted, cols = True
cm_table <- table(Predicted = pred, True = y)

# data frame for kable
cm_df <- as.data.frame.matrix(cm_table)  # keeps square format
cm_df <- cbind(Predicted = rownames(cm_df), cm_df)
rownames(cm_df) <- NULL

kable(cm_df,
      caption = "Confusion Matrix for Naive Bayes Classification \\label{tab:confusionMatrix}",
      format = "latex",
      align = c("c", rep("c", ncol(cm_df)-1))) %>%
  kable_styling(latex_options = c("HOLD_position"), full_width = FALSE) %>%
  row_spec(0, bold = TRUE) %>%
  column_spec(1:ncol(cm_df), extra_css = "font-family: serif;")
```

```{r fig.cap="Naive Bayes Classification Statistics", echo=FALSE}
# Overall statistics
overall_df <- as.data.frame(t(cm$overall))
overall_df$Class <- "Overall"

# Transpose correctly: stats as rows, class as column
overall_df_t <- as.data.frame(t(overall_df[,-ncol(overall_df)]))
colnames(overall_df_t) <- overall_df$Class
overall_df_t <- cbind(Statistic = rownames(overall_df_t), overall_df_t)  # only one Statistic column
rownames(overall_df_t) <- NULL
...
kable(overall_df_t,
  ...
  kable_styling(latex_options = c("HOLD_position", "scale_down"),
                full_width = FALSE) %>%
  row_spec(0, bold = TRUE) %>%
  column_spec(1:2, extra_css = "font-family: serif;")
```
