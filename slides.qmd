...
- Certain variables highly correlated:
  ...
  - Potassium (K) and Bicarbonate (HCO)
  - Sodium (Na) and Bicarbonate (HCO)
- Others moderately correlated. 

:::

:::{.column}

```{r corrplot, include=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.width=6, fig.height=6, dpi=100}
library(corrplot)
library(dplyr)
library(GGally)
...
```{r echo=FALSE, include=TRUE, fig.show='hold', warning=FALSE, message=FALSE, fig.width=12, fig.height=8}
faresult <- factanal(X.log, factors=5)
print(fa.diagram(faresult$loadings, digits=4))
```
