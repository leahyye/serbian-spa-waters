...
# Get loadings from PCA (PC1–PC4)
loadings <- pca_fit$rotation[, 1:4]
...
# Bold values where abs(loading) > 0.3
loadings_df <- loadings_df %>%
  mutate(across(starts_with("PC"), 
                ~ cell_spec(round(., 3), bold = abs(.) > 0.3)))

# Create and style the HTML table
kable(loadings_df, format = "html", escape = FALSE,
